...
def callbacks(call):
    ...
    if call.data == "menu_status":
        ...
        try:
            ...
            texto = (
                f"⚠️ Erro ao acessar agenda de *{nome_agenda}*\n\n"
                f"`{str(e)}`"
            )

        markup = InlineKeyboardMarkup()
        markup.add(
            InlineKeyboardButton("⬅️ Voltar", callback_data="menu_agendas_admin")
        )
